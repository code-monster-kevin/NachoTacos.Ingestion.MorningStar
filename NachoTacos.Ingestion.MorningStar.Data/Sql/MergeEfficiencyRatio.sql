/****** Object:  StoredProcedure [dbo].[MergeEfficiencyRatio]    Script Date: 1/9/2020 5:02:15 PM ******/
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'MergeEfficiencyRatio')
DROP PROCEDURE [dbo].[MergeEfficiencyRatio]
GO
...
-- Description:	Merge data from temporary job table [MStar].[TEfficiencyRatio]
--				to master table [MStar].[MEfficiencyRatio]
-- Example: EXECUTE MergeEfficiencyRatio
...
-- =============================================
CREATE PROCEDURE MergeEfficiencyRatio
AS
BEGIN
	SET NOCOUNT ON;
	...
	(SELECT GI.[ExchangeId]
      ...
      ,ER.[AVG5YrsROIC]
      ,ER.[NormalizedROIC]
	  FROM [MStar].[TEfficiencyRatio] ER

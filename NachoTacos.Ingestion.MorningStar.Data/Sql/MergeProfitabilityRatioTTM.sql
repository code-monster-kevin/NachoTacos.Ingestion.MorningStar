/****** Object:  StoredProcedure [dbo].[MergeProfitabilityRatioTTM]    Script Date: 1/9/2020 5:02:15 PM ******/
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'MergeProfitabilityRatioTTM')
DROP PROCEDURE [dbo].[MergeProfitabilityRatioTTM]
GO
...
-- Description:	Merge data from temporary job table [MStar].[TProfitabilityRatioTTM]
--				to master table [MStar].[MProfitabilityRatioTTM]
-- Example: EXECUTE MergeProfitabilityRatioTTM
...
-- =============================================
CREATE PROCEDURE MergeProfitabilityRatioTTM
AS
BEGIN
	SET NOCOUNT ON;
	...
	FROM [MStar].[TProfitabilityRatioTTM] CF
	JOIN [dbo].[IngestionTasks] IT ON IT.[IngestionTaskId] = CF.[IngestionTaskId]
	WHERE IT.[IsProcessed] = 0;


	MERGE INTO [MStar].[MProfitabilityRatioTTM] AS TargetTable
	USING
	(SELECT GI.[ExchangeId]
      ...
      ,PR.[AccessionNumber]
      ,PR.[FormType]
	  FROM [MStar].[TProfitabilityRatioTTM] PR
